...
plt.scatter(df_team1['x'], df_team1['y'], marker='x',
            label='Posição', color='blue')
for i, row in df_team1.iterrows():
    plt.text(row['x'], row['y'], f"{
             row['timestamp']:.0f}", fontsize=10, color='yellow')
...
plt.savefig('Maps/Jungler Blue Side.png', bbox_inches='tight')
plt.close()

# Criar o gráfico para o Red Side
plt.figure(figsize=(12, 8))
plt.imshow(background_image, extent=[0, 15000, 0, 15000], aspect='auto')
plt.scatter(df_team2['x'], df_team2['y'],
            marker='x', label='Posição', color='red')
...
plt.grid(False)
plt.tight_layout(rect=[0, 0, 0.85, 1])  # Ajustar layout para incluir a legenda
plt.savefig('Maps/Jungler Red Side.png', bbox_inches='tight')
plt.close()
